...
def get_saturated(gate, left_thresh, right_thresh):
    ...
    for i in range(N_LAYERS):  # for each layer
        ...
        left_tmp = np.sum(left_tmp.cpu().numpy(), 0) / SEQ_LEN  # boradcasting
        right_tmp = np.sum(right_tmp.cpu().numpy(), 0) / SEQ_LEN
        # add to a list
...
for i in range(N_LAYERS):
    ...
    u_gates, r_gates = [], []
    for x, y in tel:
        extracted = net.extract_gates(x)
        if MODEL_NAME == 'lstm':
            input_gate, forget_gate, cell_gate, output_gate = extracted[2][i]
            c_states.extend([p.data.cpu().squeeze().numpy().tolist()] for p in extracted[1][i])
